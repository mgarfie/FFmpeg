...
ACTIVATION_FILE = os.path.join(os.path.expanduser("~"), ".my_video_splitter_activation")
...
def save_activation_code(code):
    ...
    try:
        ...
        if os.name == 'nt':
            import ctypes
            ctypes.windll.kernel32.SetFileAttributesW(ACTIVATION_FILE, 2)  # 2 = FILE_ATTRIBUTE_HIDDEN
...
def check_activation():
    ...
    root = tk.Tk()
    root.withdraw()
    for _ in range(3):  # 最多允许输3次
        code = simpledialog.askstring("激活码", "请输入激活码:", show='*')
        if code == VALID_ACTIVATION_CODE:
            save_activation_code(code)
            messagebox.showinfo("激活成功", "激活码验证通过！")
            root.destroy()
            return True
        else:
            messagebox.showerror("错误", "激活码错误，请重试。")
    root.destroy()
    messagebox.showerror("错误", "激活失败，程序即将退出。")
    sys.exit()
...
class VideoSplitterApp:
    ...
    def create_widgets(self):
        ...
        tk.Button(root, text="关闭程序", command=self.close_program).grid(row=4, column=0, pady=10)
